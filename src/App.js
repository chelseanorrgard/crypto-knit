...
const encryptVigenere = (text, key = 'KEY') => {
  key = key.toUpperCase(); // Normalize key
  let result = '';
  let keyIndex = 0;
...
const encryptPlayfair = (text, key = 'KEYWORD') => {
  ...
  for (let [a, b] of digraphs) {
    const [r1, c1] = findPos(a);
    const [r2, c2] = findPos(b);

    if (r1 === r2) {
      result += keySquare[r1 * 5 + (c1 + 1) % 5];
      result += keySquare[r2 * 5 + (c2 + 1) % 5];
    } else if (c1 === c2) {
      result += keySquare[((r1 + 1) % 5) * 5 + c1];
      result += keySquare[((r2 + 1) % 5) * 5 + c2];
    } else {
      result += keySquare[r1 * 5 + c2];
      result += keySquare[r2 * 5 + c1];
    }
  }
